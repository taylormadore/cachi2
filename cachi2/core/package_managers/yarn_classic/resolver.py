...
def _get_packages_from_lockfile(source_dir: RootedPath) -> list[YarnClassicPackage]:
    """Return a list of Packages for all dependencies in yarn.lock."""
    lockfile_path = source_dir.join_within_root("yarn.lock")
    lockfile = Lockfile.from_file(lockfile_path)
    pyarn_packages: list[PYarnPackage] = lockfile.packages()
